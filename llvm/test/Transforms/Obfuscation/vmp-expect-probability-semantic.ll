...
; Reference omits expect.with.probability: host lli/codegen cannot select it.
; Runtime value of the intrinsic is always operand 0, so a plain branch matches.
...
entry:
  ...
  %cmp = icmp sgt i32 %x, 0
  %likely = call i1 @llvm.expect.with.probability.i1(i1 %cmp, i1 true, double 9.000000e-01)
...
; CHECK: vmp.dispatch:
; CHECK-NOT: expect.with.probability
