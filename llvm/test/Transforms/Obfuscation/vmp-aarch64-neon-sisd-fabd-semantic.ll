; Restricted AArch64 scalar FABD via CallDescriptor / scalar
; float VRegs:
;   llvm.aarch64.sisd.fabd
;     AdvSIMD_2Scalar_Float: anyfloat (match, match)
;     ISel SIMDFPThreeScalar FABD, baseline HasNEON:
;       scalar float / double
;     plus HasFullFP16:
;       scalar half
; Clang vabds_f32 / vabdd_f64 / vabdh_f16.  Must not lower to
; fabs(fsub).  Vector neon.fabd is
; vmp-aarch64-neon-fabd-semantic.ll and must not stay here as a
; well-formed skip (it would virtualize).  v1f64 is neon.fabd,
; not this ID.  Last-token +fullfp16 required for half; f32/f64
; need no extra token.  +fp16fml does not count.  FastMathFlags
; are rejected by the existing float-call FMF gate.
...
define float @protected_sisd_fabd_f32(float %a, float %b) noinline optnone {
entry:
  call void @hikari_vmp()
  %r = call float @llvm.aarch64.sisd.fabd.f32(float %a, float %b)
  ret float %r
}

define double @protected_sisd_fabd_f64(double %a, double %b) noinline optnone {
entry:
  call void @hikari_vmp()
  %r = call double @llvm.aarch64.sisd.fabd.f64(double %a, double %b)
  ret double %r
}

define half @protected_sisd_fabd_f16(half %a, half %b) noinline optnone "target-features"="+fullfp16" {
entry:
  call void @hikari_vmp()
  %r = call half @llvm.aarch64.sisd.fabd.f16(half %a, half %b)
  ret half %r
}

define half @protected_sisd_fabd_last_fullfp16(half %a, half %b) noinline optnone "target-features"="+neon,+fp16fml,+fullfp16" {
entry:
  call void @hikari_vmp()
  %r = call half @llvm.aarch64.sisd.fabd.f16(half %a, half %b)
  ret half %r
...
define float @protected_sisd_fabd_bounds_f32() noinline optnone {
entry:
  call void @hikari_vmp()
  %r = call float @llvm.aarch64.sisd.fabd.f32(float -0.0, float 0.0)
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = call nnan ninf float @llvm.aarch64.sisd.fabd.f32(float %a, float %b)
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = call bfloat @llvm.aarch64.sisd.fabd.bf16(bfloat %a, bfloat %b)
  ret bfloat %r
}
...
entry:
  call void @hikari_vmp()
  %r = call fastcc float @llvm.aarch64.sisd.fabd.f32(float %a, float %b)
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = musttail call float @llvm.aarch64.sisd.fabd.f32(float %a, float %b)
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = call float @llvm.aarch64.sisd.fabd.f32(float %a, float %b) [ "deopt"(i32 0) ]
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = call float @llvm.aarch64.sisd.fabd.f32(float %a, float %b) noreturn
  ret float %r
}
...
entry:
  call void @hikari_vmp()
  %r = call float @llvm.aarch64.sisd.fabd.f32(float %a, float %b) returns_twice
  ret float %r
}
...
entry:
  ...
  %r0 = call float @protected_sisd_fabd_f32(float %s0, float %s1)
  ...
  %r1 = call double @protected_sisd_fabd_f64(double %d0, double %d1)
  ...
  %r2 = call half @protected_sisd_fabd_f16(half %h0, half %h1)
  store volatile half %r2, ptr @sink_f16, align 2
  %r3 = call half @protected_sisd_fabd_last_fullfp16(half %h0, half %h1)
  store volatile half %r3, ptr @sink_f16, align 2
  %r4 = call float @protected_sisd_fabd_bounds_f32()
...
; SKIP-NOT: Skipping VMP on protected_sisd_fabd_f32:
; SKIP-NOT: Skipping VMP on protected_sisd_fabd_f64:
; SKIP-NOT: Skipping VMP on protected_sisd_fabd_f16:
; SKIP-NOT: Skipping VMP on protected_sisd_fabd_last_fullfp16:
; SKIP-NOT: Skipping VMP on protected_sisd_fabd_bounds_f32:

; VIRT: define float @protected_sisd_fabd_f32({{.*}} #[[PROT:[0-9]+]] {
; VIRT: vmp.dispatch:
; VIRT: call float @llvm.aarch64.sisd.fabd.f32(
; VIRT: define double @protected_sisd_fabd_f64({{.*}} #[[PROT]] {
; VIRT: vmp.dispatch:
; VIRT: call double @llvm.aarch64.sisd.fabd.f64(
; VIRT: define half @protected_sisd_fabd_f16({{.*}} #[[PROTH:[0-9]+]] {
; VIRT: vmp.dispatch:
; VIRT: call half @llvm.aarch64.sisd.fabd.f16(
; VIRT: define half @protected_sisd_fabd_last_fullfp16({{.*}} #[[LAST:[0-9]+]] {
; VIRT: vmp.dispatch:
; VIRT: call half @llvm.aarch64.sisd.fabd.f16(
; VIRT: define float @protected_sisd_fabd_bounds_f32({{.*}} #[[PROT]] {
; VIRT: vmp.dispatch:
; VIRT: call float @llvm.aarch64.sisd.fabd.f32(
...
; AARCH64-ASM-DAG: {{^[[:space:]]*}}fabd{{[ \t]}}{{s[0-9]+}}
; AARCH64-ASM-DAG: {{^[[:space:]]*}}fabd{{[ \t]}}{{d[0-9]+}}
; AARCH64-ASM-DAG: {{^[[:space:]]*}}fabd{{[ \t]}}{{h[0-9]+}}
