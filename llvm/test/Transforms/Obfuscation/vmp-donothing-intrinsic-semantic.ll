; llvm.donothing(): documented no-op.  C, exact non-vararg void(),
; zero args.  Replay via CallDescriptor; never fold or delete.
; Ordinary tail accepted and replayed as TCK_None.  sideeffect is a separate whitelist.
;
; Host IntrinsicLowering has no donothing case (pure interpreter
...
entry:
  call void @hikari_vmp()
  call void @llvm.donothing() noreturn
  ret void
}
...
entry:
  call void @hikari_vmp()
  musttail call void @llvm.donothing()
  ret void
}
...
entry:
  call void @hikari_vmp()
  call void @llvm.donothing() [ "deopt"(i32 0) ]
  ret void
}
...
entry:
  call void @hikari_vmp()
  call fastcc void @llvm.donothing()
  ret void
}
...
; SKIP: seeded with:
; SKIP-DAG: Skipping VMP on unsupported_malformed: unsupported donothing
; SKIP-DAG: Skipping VMP on unsupported_as1_arg: unsupported argument type
; SKIP-DAG: Skipping VMP on unsupported_musttail: musttail call
; SKIP-DAG: Skipping VMP on unsupported_bundle: unsupported donothing
; SKIP-DAG: Skipping VMP on unsupported_fastcc: unsupported donothing
...
; donothing is re-emitted as the original intrinsic (never deleted).
; VIRT-DAG: call void @llvm.donothing()
; VIRT: }
...
; VIRT-NOT: vmp.dispatch
; VIRT: musttail call void @llvm.donothing()
...
; donothing is a no-op; llc must still produce a function.
; AARCH64-ASM: ret
